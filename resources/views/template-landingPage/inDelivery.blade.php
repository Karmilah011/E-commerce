...
@section('content')
    ...
    <section class="shopping-cart spad">
        <div class="container">
            @foreach ($processCodeInDelivery as $processCode)
                <div class="row mb-5">
                    <div class="col-lg-12">
                        <div class="shopping__cart__table">
                            <table>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <a href="{{route('updateStatus',['status'=>'finish','process_code'=>$processCode->process_code])}}" class="btn btn-dark">Order Completed</a>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <span>ID Transaction : <a href="{{route('inDelivery',['status'=>'packed','process_code'=>$item->process_code])}}">{{$item->process_code}}</a></span> <br>
                        </div>
                    </div>
                    <hr>
                </div>
            @endforeach
            @if (count($processCodeInDelivery) == 0)
